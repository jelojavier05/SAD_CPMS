            ...
            <table class="striped white" style="border-radius:10px;" id="dataTable">
              <thead>
                <tr>
                  ...
                  <th class="grey lighten-1">Expiration</th>								
                </tr>
              </thead>
              <tbody>
              </tbody>
...
<!--modal renew license-->
  <div id="modalRenewGuardLicense" class="modal modal-fixed-footer ci" style="overflow:hidden; width:700px;max-height:100%; height:400px; margin-top:60px;">
    ...
    <div class="modal-footer ci" style="background-color: #00293C;">
      <button class="btn green waves-effect waves-light" name="" id = "btnGuardLicenseOkay" style="margin-right: 30px;">OK
      </button>
    </div>
  </div>
...
$(document).ready(function(){
  ...
  $.ajax({
    type: "GET",
    url: "{{action('GuardLicensesController@getGuardToBeExpired')}}",
    success: function(data){
      ...
      $.each(data, function (index, value){
        radio = '<input name="guard" class = "with-gap" type="radio" id="radio'+value.intGuardID+'" value = '+value.intGuardID+' /> <label for="radio'+value.intGuardID+'"></label>';
  ...
  $('#btnGuardLicenseOkay').click(function(){
    if (checkDate()){
      var guardID = $('input[name=guard]:checked').val();
      $.ajax({
        type: "POST",
        url: "{{action('GuardLicensesController@updateGuardLicense')}}",
        ...
        success: function(data){
          swal({
            title: "Success!",
            text: "Guard License Updated.",
            type: "success"
          },
          function(){
            window.location.href = '{{ URL::to("/guardLicenses") }}';
          });
        },
        error: function(data){
          var toastContent = $('<span>Error Database.</span>');
  ...
  function hasGuardChecked(){
    if ($('input[name=guard]').is(':checked')){
      return true;
    }else{
